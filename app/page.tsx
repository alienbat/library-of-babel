...
export default function Home() {
  ...
  return <main className={playing?'game playing':'game'}>
    ...
    {!playing&&<section className="menu" aria-label={entered?'Paused':'Enter the library'}>
      ...
      <div className="instructions"><span><kbd>W A S D</kbd> Move</span><span><kbd>MOUSE</kbd> Look</span><span><kbd>SHIFT</kbd> Move faster</span><span><kbd>SPACE</kbd> Toggle flight</span><span><kbd>ESC</kbd> Pause</span></div>
      <p className="mobile-instructions">Use the left pad to move. Drag on the right to look. Tap Fly to take off.</p>
    </section>}
    {playing&&<><span className="crosshair" aria-hidden="true"/><div className="walking-hint">{stats.mode==='flying'?'WASD follows your view · Look up/down to climb or descend · Space to fall':stats.mode==='falling'?'Falling · Space to fly again':drag?'Drag to look · WASD to walk · Space to fly':'WASD to walk · Mouse to look · Space to fly'}</div><div className="touch-pad" aria-label="Movement controls">{(['forward','left','back','right'] as const).map((direction,i)=><button key={direction} className={direction} aria-label={`Walk ${direction}`} onPointerDown={e=>{e.currentTarget.setPointerCapture(e.pointerId);game.current?.touchMove(direction,true);}} onPointerUp={()=>game.current?.touchMove(direction,false)} onPointerCancel={()=>game.current?.touchMove(direction,false)}>{['↑','←','↓','→'][i]}</button>)}</div></>}
