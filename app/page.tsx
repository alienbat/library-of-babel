...
export default function Home() {
  const viewport=useRef<HTMLDivElement>(null), game=useRef<GameHandle|null>(null);
  ...
  const [stats,setStats]=useState({floor:0,distance:0}),[drag,setDrag]=useState(false);
  useEffect(()=>{
    let disposed=false;
    import('../lib/game/engine').then(({createGame})=>{
      if(disposed||!viewport.current)return;
      try {game.current=createGame(viewport.current,{onPause:()=>setPlaying(false),onStats:setStats,onFallback:()=>setDrag(true),onError:setError});setReady(true);}
  ...
  return <main className={playing?'game playing':'game'}>
    ...
    {!playing&&<section className="menu" aria-label={entered?'Paused':'Enter the library'}>
      ...
      {settings&&<div className="settings"><label>Field of view <span>{fov}°</span><input type="range" min="55" max="100" value={fov} onChange={e=>setFov(+e.target.value)}/></label><label>Mouse sensitivity <span>{sensitivity.toFixed(1)}</span><input type="range" min="0.3" max="2.5" step="0.1" value={sensitivity} onChange={e=>setSensitivity(+e.target.value)}/></label><label className="inline-label">Gentle walking motion<input type="checkbox" checked={motion} onChange={e=>setMotion(e.target.checked)}/></label><label className="inline-label">Detail<select value={quality} onChange={e=>setQuality(e.target.value)}><option value="high">High</option><option value="low">Low</option></select></label><button className="reset" onClick={()=>{game.current?.reset();setStats({floor:0,distance:0});}}>Return to starting point</button></div>}
      <div className="instructions"><span><kbd>W A S D</kbd> Walk</span><span><kbd>MOUSE</kbd> Look</span><span><kbd>SHIFT</kbd> Walk faster</span><span><kbd>ESC</kbd> Pause</span></div>
      <p className="mobile-instructions">Use the left pad to walk. Drag on the right to look.</p>
    </section>}
    {playing&&<><span className="crosshair" aria-hidden="true"/><div className="walking-hint">{drag?'Drag to look · WASD to walk · Esc to pause':'WASD to walk · Mouse to look · Shift to walk faster'}</div><div className="touch-pad" aria-label="Movement controls">{(['forward','left','back','right'] as const).map((direction,i)=><button key={direction} className={direction} aria-label={`Walk ${direction}`} onPointerDown={e=>{e.currentTarget.setPointerCapture(e.pointerId);game.current?.touchMove(direction,true);}} onPointerUp={()=>game.current?.touchMove(direction,false)} onPointerCancel={()=>game.current?.touchMove(direction,false)}>{['↑','←','↓','→'][i]}</button>)}</div></>}
    <footer><span>{playing?'WALKING EXPLORATION':'AN UNOFFICIAL LITERARY EXPLORATION'}</span><div><span>LEVEL <b>{stats.floor===0?'ARRIVAL':`${stats.floor>0?'+':''}${stats.floor}`}</b></span><span><b>{stats.distance.toLocaleString()}</b> m walked</span></div></footer>
  </main>;
}
