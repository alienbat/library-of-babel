...
export function staircase(x:number,y:number,side:number,limits:WorldLimits={}){
  ...
  walls.push([x+8,wallY,side*(OUTER+.48),8,HEIGHT,.16],
    [x+8,wallY,side*(OUTER+3.8),14.2,HEIGHT,.2],
    [x+1,wallY,side*(OUTER+1.9),.2,HEIGHT,4],
    [x+15,wallY,side*(OUTER+1.9),.2,HEIGHT,4]);
  ...
  for(const landing of [2.5,13.5]){
    walls.push([x+landing,y+2.6+lintelHeight/2,side*(OUTER+.48),3,lintelHeight,.16]);
  ...
  if(bottom){
    ...
    walls.push([x+12,wallY,side*(OUTER+2.05),.16,HEIGHT,3.5]);
  }
  if(top){
    floors.push([x+8,y+HEIGHT-.17,side*(OUTER+1.9),14,.34,3.8]);
    walls.push([x+4,wallY,side*(OUTER+2.05),.16,HEIGHT,3.5]);
  }
