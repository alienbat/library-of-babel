  ...
  function material(params:T.MeshStandardMaterialParameters){
    ...
    m.onBeforeCompile=shader=>{
      ...
      shader.fragmentShader=shader.fragmentShader.replace('#include <color_fragment>',`#include <color_fragment>
        ...
        vec3 positive=texture(bakedPositive,uvw).rgb*${RANGE}.0;
        vec3 negative=texture(bakedNegative,uvw).rgb*${RANGE}.0;
        float irradiance=dot(n*n,mix(negative,positive,step(vec3(0.0),n)));
        // Enclosed rooms have their own diffuse ceiling illumination, without gallery light leaking through walls.
        float room=step(${OUTER+.4},abs(vBakedPosition.z));
        irradiance=mix(irradiance,.85+.35*max(n.y,0.0)-.2*max(-n.y,0.0),room);
        diffuseColor.rgb*=irradiance*vec3(1.0,.97,.89);
      `);
    };
    m.customProgramCacheKey=()=> 'baked-gallery-volume-v1';return m;
  }
  return {material,positive:pos,negative:neg,dispose(){pos.dispose();neg.dispose();}};
}
