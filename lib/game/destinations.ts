import {PERIOD,HEIGHT,INNER,type Position,type WorldLimits} from './physics.ts';
...
export function destinationState(destination:Destination):{position:Position;limits:WorldLimits;yaw:number;pitch:number}{
  ...
  if(destination!=='arrival'){
    if(destination.endsWith('left'))limits.minX=0;else limits.maxX=PERIOD;
    if(destination.startsWith('bottom'))limits.minY=0;else limits.maxY=HEIGHT-.34;
  }
  return {position:{x:destination.endsWith('right')?PERIOD-15:30,y:0,z:INNER+1.7},limits,yaw:destination.endsWith('left')?.8:destination.endsWith('right')?-.8:-.88,pitch:destination==='arrival'?-.04:destination.startsWith('top')?.18:-.12};
}
