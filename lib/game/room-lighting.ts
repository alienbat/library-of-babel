...
export const ROOM_LIGHTS=[
  ...
  {x:8,y:HEIGHT*1.5-.43,z:2.1,room:0},
...
export function bakeRoomLighting(){
  ...
  for(let z=0;z<nz;z++)for(let y=0;y<ny;y++)for(let x=0;x<nx;x++){
    ...
    const floor=room===0?Math.floor((py-ramp(px)+.001)/HEIGHT)*HEIGHT:0;
    ...
    for(const lamp of ROOM_LIGHTS){
      ...
      const ly=lamp.y+(room===0?floor+(lamp.x>=12?HEIGHT:0):0);
    ...
    const floorY=room===0?py-floor-ramp(px):py;
