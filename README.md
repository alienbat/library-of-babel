...
Debug teleport: press backtick (`) to open the corner/arrival destination menu. These moves reset the local LEVEL reference but preserve lifetime journey distance and elapsed time. Corner boundaries clip geometry and the analytic horizon. Original arrival returns to this journey's seeded starting region.

Journey progress is stored in this browser under `babel-journey-v1`: exact global frame offsets, local position, view, flight/fall state, millimetre distance ledger, artificial elapsed milliseconds and first-entry timestamp. Save progress is available in ESC; autosave runs every five minutes, after timed walking and on page exit. A saved journey resumes on reload; a new journey chooses a deterministic random interior origin. East is increasing horizontal coordinates on both galleries, with directional signs baked into their walls.

T contains search, bookmarks and timed walking. Up/Down/East/West walks offer 1, 1,000, 1,000,000 or 1,000,000,000 years at 1.7 m/s for ten hours per day, with 365-day years. Exact worker arithmetic advances the global address and stops at boundaries, proportionally truncating elapsed time and distance. Each walk finishes on a gallery floor (vertical floor alignment can differ by less than one floor height). The persisted distance includes ordinary movement and timed walks; debug teleport adds no distance. Library time includes offline real time plus artificial walking time, with arbitrary-size years and a non-leap calendar remainder. Progress is local to the browser, not an account cloud save.
...
Shelf detail no longer follows the player's current level. A separate cache selects whole shelf bays by 3D distance to their bounds (12 m, with the selection eye snapped to a 1 m grid). Nearby floors receive real book volumes, boards and plain backing; cached bays keep their absolute local level for opened-book colours. Only entering/leaving bays are rebuilt. The static gallery window can still translate during flight without dragging nearby book identities onto another floor.

Beyond that radius, shared shelf boxes add a cheap 6 cm analytical relief layer for the board fronts and book spines, up to 500 m. It blends to the ordinary atlas at 450–500 m and earlier when board edges become subpixel. This is an approximation of shelf depth, not individual book geometry throughout a 500 m sphere. Infinite-gallery rendering and its view distance are unchanged.
